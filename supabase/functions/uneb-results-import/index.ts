// Thin alias for knec-results-import with body_code locked to "uneb".
// The underlying importer already supports KCSE/KCPE/UNEB/NECTA/REB
// via a body_code param; this wrapper exists so the contract reads
// cleanly from the UI and from external callers.
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const { data: student } = await supa.from("students").select("id")
        .eq("tenant_id", tenant_id).eq("uneb_index_number", indexNum).maybeSingle();
    ...
    await supa.from("compliance_exports_log").insert({
      tenant_id, kind: "uneb_results_import",
      params: { matched, unmatched, total: matched + unmatched }, row_count: matched,
    });

    return new Response(JSON.stringify({ body: "uneb", matched, unmatched, sample }), {
