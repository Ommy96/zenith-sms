...
## Timetable & Examinations rebuild
- **Timetable** (`src/pages/Timetable.tsx`): Tabbed workflow — Schedule / Class views / Teacher views / Room views / AI generator / Conflicts / Setup. "Generate with AI" header CTA (gated on prerequisites). Setup tab seeds standard Kenyan school day; periods listed with delete + bell-schedule preview. Conflicts tab lists term-wide teacher double-bookings, room clashes and unstaffed slots with Resolve action. Teacher and room tabs show workload + utilization stats.
- **Examinations** (`src/pages/Examinations.tsx`): Removed empty KPI cards; replaced with contextual banner (in-progress marking, next exam, or seed prompt). Tabs — Calendar / Grade entry / Results / Analytics / Templates. Calendar groups exams by Planned / In progress / Completed with progress bars and one-click "Seed term" Kenyan pattern (CAT week 2, Midterm week 6, End-Term week 12). Grade entry is a spreadsheet with CBC performance-level dropdowns when `tenant.curriculum='cbc'`, else numeric inputs with max-marks validation. Results view shows subject performance, top performers, intervention list, and a publish-reports button wired to `generate-report-cards`. Analytics tab plots mean-score trends with a simple AI insight.
- **Setup checklist** (`src/hooks/useSetupChecklist.ts`): Added weighted contributions for periods, timetable slots and exam scheduling.

## Security: M-Pesa C2B callback hardening

- **Removed tenant UUID from callback URL.** `mpesa-c2b-callback` no longer reads `?school=` / `?tenant=` query parameters. Tenant is resolved server-side from the `BusinessShortCode` in the Safaricom payload via `mpesa_config.shortcode`. Eliminates information disclosure of tenant IDs on a public endpoint and prevents spoofing the routing target.
