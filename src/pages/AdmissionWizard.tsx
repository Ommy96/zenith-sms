...
export default function AdmissionWizard() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // 1. Insert student — admission_number is auto-generated by the
      //    BEFORE INSERT trigger (_tg_student_admission_number) when omitted.
      ...
      const admission_number = student.admission_number;

      // 2. Guardians — reuse an existing guardian with the same phone, else create.
      for (const g of guardians) {
        let guardianId: string | null = null;
        if (g.phone_primary) {
          const { data: existing } = await supabase
            .from("guardians").select("id")
            .eq("tenant_id", tenantId).eq("phone_primary", g.phone_primary)
            .maybeSingle();
        ...
        const { error: linkErr } = await supabase.from("student_guardians").upsert({
          tenant_id: tenantId,
      ...
      for (const f of files) {
        const path = `${tenantId}/students/${student.id}/${Date.now()}-${f.name}`;
